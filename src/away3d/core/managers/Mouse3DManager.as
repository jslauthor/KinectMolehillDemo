package away3d.core.managers
{
	...
	public class Mouse3DManager
	{
		...
		private function onClick(event : MouseEvent) : void
		{
			...
			getObjectHitData(event.localX, event.localY);
		 ...
		 */
		private function getObjectHitData(x:Number = NaN, y:Number = NaN) : void
		{
			if (isNaN(x)) x = _stage.mouseX;
			if (isNaN(y)) y = _stage.mouseY;
			...
			if (collector.numMouseEnableds > 0) {
				_hitTestRenderer.update(x/_stage.stageWidth, y/_stage.stageHeight, collector);
